...
typedef enum {
  ...
  C_HTTP1_1,
  C_HTTP2,
  C_HTTP2_NO_SERVER_PUSH,
  C_HTTP2_PESUDO_HEADERS_ORDER,
...
static const struct LongShort aliases[]= {
  ...
  {"cert-compression",           ARG_STRG, ' ', C_CERT_COMPRESSION},
  ...
  {"http2-no-server-push",       ARG_BOOL, ' ', C_HTTP2_NO_SERVER_PUSH},
  {"http2-pseudo-headers-order", ARG_STRG, ' ', C_HTTP2_PESUDO_HEADERS_ORDER},
  ...
  {"tls-session-ticket",         ARG_BOOL, ' ', C_TLS_SESSION_TICKET},
  {"tls-permute-extensions",     ARG_BOOL, ' ', C_TLS_PERMUTE_EXTENSIONS},
  ...
  *usedarg = FALSE; /* default is that we don't use the arg */
  ...
  do {
    ...
    case C_ALPS: /* --alps */
      config->alps = toggle;
      break;
    case C_CERT_COMPRESSION: /* --cert-compression */
      err = getstr(&config->ssl_cert_compression, nextarg, DENY_BLANK);
      break;
    case C_TLS_SESSION_TICKET: /* --tls-session-ticket */
      config->noticket = (!toggle)?TRUE:FALSE;
      break;
    case C_HTTP2_PESUDO_HEADERS_ORDER: /* --http2-pseudo-headers-order */
      err = getstr(&config->http2_pseudo_headers_order, nextarg, DENY_BLANK);
      break;
    case C_HTTP2_NO_SERVER_PUSH: /* --http2-no-server-push */
      config->http2_no_server_push = toggle;
      break;
    case C_TLS_PERMUTE_EXTENSIONS: /* --tls-permute-extensions */
